...
if clientID!=-1:
    ...
    [returnCode,joint]=sim.simxGetObjectHandle(clientID,'Revolute_joint',sim.simx_opmode_blocking)
    direction_forward = True

    while (True):
        if (sim.simxGetJointPosition(clientID, joint, sim.simx_opmode_blocking)[1] > math.pi/2):
            direction_forward = False

        if (sim.simxGetJointPosition(clientID, joint, sim.simx_opmode_blocking)[1] < 0):
            direction_forward = True
        
        if (direction_forward):
            sim.simxSetJointTargetVelocity(clientID, joint, math.pi/10, sim.simx_opmode_oneshot)
        else:
            sim.simxSetJointTargetVelocity(clientID, joint, -math.pi/10, sim.simx_opmode_oneshot)
